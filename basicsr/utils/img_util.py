...
def img2tensory(imgs, bgr2ycbcr=True, float32=True):
    """Numpy array to tensor.

    Args:
        imgs (list[ndarray] | ndarray): Input images.
        bgr2ycbcr (bool): Whether to change BGR to YCbCr.
        float32 (bool): Whether to change to float32.
    ...
    def _totensor(img, bgr2ycbcr, float32):
        if img.shape[2] == 3 and bgr2ycbcr:
    ...
    if isinstance(imgs, list):
        return [_totensor(img, bgr2ycbcr, float32) for img in imgs]
    else:
        return _totensor(imgs, bgr2ycbcr, float32)
